...
String classifyDart(String src) {
  ...
  while (token.type != TokenType.EOF) {
    ...
    if (comment.trim().length > 0) {
      out.write('<span class="${Classification.COMMENT}">$comment</span>');
    }
    ...
    out.write('<span class="$kind$stringClass">${escapeHtml(token.lexeme)}</span>');
    token = token.next;
  }
  // Add remaining whitespace.
  out.write(src.slice(pos, token.offset));
